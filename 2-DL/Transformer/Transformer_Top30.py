"""
BMS Predictive Maintenance - Transformer Top30 Pipeline
==================================================

OBJECTIVE:
----------
Build a Transformer (encoder-only) model that predicts SOH from
battery operational data using Top 30 features by SOH correlation.
Unlike classical ML models (RF, XGBoost), Transformer processes sequences
...
8. Transformer model training (Baseline + Tuned)
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_Transformer_Top30")
...
# Transformer sequence length: how many past cycles the model sees for each prediction
SEQ_LEN = 5
...
class SOH_Transformer(nn.Module):
    """
    Encoder-only Transformer for SOH prediction.

    Architecture:
        Input -> Linear projection -> Positional Encoding
        -> TransformerEncoder (self-attention + feed-forward)
        -> LayerNorm + Dropout (last timestep)
        -> FC head -> 1
    """
...
def train_transformer_model(model, X_train, y_train, X_test, y_test,
                     epochs=200, batch_size=16, lr=0.001, patience=30):
    """
    Trains Transformer model with early stopping.

    Args:
        model: SOH_Transformer instance
...
if __name__ == "__main__":
    print("=" * 60)
    print("BMS Predictive Maintenance - Transformer Top30 Pipeline")
    print("=" * 60)
    ...
    # ============================================================
    # STEP 8: Transformer Model Training (Baseline)
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 8: Transformer Model Training (Baseline)")
    print("=" * 60)
    ...
    print("\n  --- Baseline Transformer ---")
    print(f"  Architecture: Transformer, d_model=64, nhead=4, layers=2")
    baseline_model = SOH_Transformer(n_features, d_model=64, nhead=4, n_layers=2, dropout=0.2)
    t0 = time.time()
    ...
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X_seq)):
        cv_model = SOH_Transformer(n_features, d_model=64, nhead=4, n_layers=2, dropout=0.2)
        train_transformer_model(cv_model, X_seq[tr_idx], y[tr_idx], X_seq[te_idx], y[te_idx],
    ...
    # Save baseline model
    torch.save(baseline_model.state_dict(), os.path.join(OUTPUT_DIR, "model_transformer_baseline.pt"))
    print(f"\n[MODEL] model_transformer_baseline.pt saved")
    ...
    plt.suptitle("STEP 8 - Transformer: Train vs Test Metrics (Overfitting Check)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    ax.plot(cycles_test, y_pred_test, "r--s", label="Transformer Prediction", linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 8 - Transformer SOH Prediction\nMAE={mae_test:.3f}% | RMSE={rmse_test:.3f}% | R2={r2_test:.4f}",
                 fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title(f"STEP 8 - Transformer Residuals (max |error|={np.max(np.abs(residuals)):.3f}%)", fontweight="bold")
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title("STEP 8 - Transformer Training Loss Curve", fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    for i, (name, val, clr) in enumerate(zip(metric_names, metric_vals, bar_colors_met)):
        ...
        axes[i].grid(True, alpha=0.3, axis="y")
    plt.suptitle("STEP 8 - Transformer Test Results", fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    print(f"  Grid: {len(PARAM_GRID['d_model'])} x {len(PARAM_GRID['n_layers'])} x {len(PARAM_GRID['lr'])} "
          f"= {len(PARAM_GRID['d_model']) * len(PARAM_GRID['n_layers']) * len(PARAM_GRID['lr'])} combinations")
    ...
    for dm in PARAM_GRID["d_model"]:
        for nl in PARAM_GRID["n_layers"]:
            for lr_val in PARAM_GRID["lr"]:
                model_gs = SOH_Transformer(n_features, d_model=dm, nhead=4, n_layers=nl, dropout=0.2)
                train_transformer_model(model_gs, X_train, y_train, X_test, y_test,
                ...
                tuning_results.append({
                    "d_model": dm, "n_layers": nl, "lr": lr_val, "test_mae": gs_mae,
                })
                if gs_mae < best_mae:
                    best_mae = gs_mae
                    best_params = {"d_model": dm, "n_layers": nl, "lr": lr_val}
    ...
    print(f"\n  --- Tuned Transformer (d_model={best_params['d_model']}, "
          f"layers={best_params['n_layers']}, lr={best_params['lr']}) ---")
    tuned_model = SOH_Transformer(n_features, d_model=best_params["d_model"],
                           nhead=4, n_layers=best_params["n_layers"], dropout=0.2)
    ...
    cv_maes_tuned = []
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X_seq)):
        cv_model = SOH_Transformer(n_features, d_model=best_params["d_model"],
                            nhead=4, n_layers=best_params["n_layers"], dropout=0.2)
        train_transformer_model(cv_model, X_seq[tr_idx], y[tr_idx], X_seq[te_idx], y[te_idx],
    ...
    # Save tuned model
    torch.save(tuned_model.state_dict(), os.path.join(OUTPUT_DIR, "model_transformer_tuned.pt"))
    print(f"\n[MODEL] model_transformer_tuned.pt saved")
    ...
    for i, (name, bl_v, tn_v) in enumerate(zip(metrics_names, baseline_vals, tuned_vals)):
        ...
        for bar, val in zip(bars, [bl_v, tn_v]):
            axes[i].text(bar.get_x() + bar.get_width() / 2, bar.get_height(),
                         f"{val:.4f}", ha="center", va="bottom", fontsize=10)
        axes[i].grid(True, alpha=0.3, axis="y")
    plt.suptitle("STEP 9 - Baseline vs Tuned Transformer (Test Metrics)", fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(cycles_test, y_test, "b-o", label="Actual SOH", linewidth=2, markersize=5)
    ax.plot(cycles_test, y_pred_tuned_test, "r--s", label="Transformer Tuned Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_tuned_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 9 - Tuned Transformer SOH Prediction\n"
    ...
    # --- Result CSV ---
    result_path = os.path.join(OUTPUT_DIR, "Result_Transformer_Top30.csv")
    ...
    print(f"[CSV] Result_Transformer_Top30.csv saved")
