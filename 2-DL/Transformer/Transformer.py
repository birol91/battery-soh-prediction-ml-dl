"""
BMS Predictive Maintenance - Transformer Pipeline
============================================
...
Transformer processes sequences of cycles and can learn temporal patterns
...
6. [TBD] Transformer model training
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_Transformer")
...
class PositionalEncoding(nn.Module):
    ...
    Transformer has no notion of order — this tells it which cycle is 1st, 2nd, etc."""
...
class SOH_Transformer(nn.Module):
    ...
    Architecture:
        ...
        -> Positional Encoding
        -> TransformerEncoder (self-attention + feed-forward, n_layers)
        -> LayerNorm + Dropout (last timestep)
        -> FC head (d_model -> d_model -> 1)

    Self-attention allows each cycle to attend to all other cycles
    in the sequence, capturing both local and long-range dependencies.
    """
    ...
    def forward(self, x):
        ...
        h = self.encoder(h)           # self-attention layers
...
if __name__ == "__main__":
    print("=" * 60)
    print("BMS Predictive Maintenance - Transformer Pipeline")
    print("=" * 60)
    ...
    # ---- STEP 6: Feature Selection (All Features — Transformer selects internally) ----
    print("\n" + "=" * 60)
    print("STEP 6: Feature Selection (All Features)")
    print("=" * 60)
    feature_cols = all_feature_cols  # Use all features — no manual selection
    print(f"  Total features: {len(feature_cols)}")
    print(f"  No manual selection — Transformer handles feature importance via learned weights")
    ...
    # --- Result CSV ---
    result_path = os.path.join(OUTPUT_DIR, "Result_Transformer.csv")
    ...
    print(f"[CSV] Result_Transformer.csv saved")
