"""
BMS Predictive Maintenance - GRU Top30 Pipeline
==================================================

OBJECTIVE:
----------
Build an GRU (Gated Recurrent Unit) model that predicts SOH from
battery operational data using Top 30 features by SOH correlation.
Unlike classical ML models (RF, XGBoost), GRU processes sequences
...
8. GRU model training (Baseline + Tuned)
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_GRU_Top30")
...
# GRU sequence length: how many past cycles the model sees for each prediction
SEQ_LEN = 5
...
class SOH_GRU(nn.Module):
    """
    Bidirectional GRU for SOH prediction.

    Architecture:
        Input (seq_len, n_features)
        -> Bidirectional GRU (2 layers)
        -> LayerNorm + Dropout
        -> FC head (hidden*2 -> hidden -> 1)

    GRU is a simplified version of LSTM with 2 gates (reset, update)
    instead of 3 (forget, input, output). Fewer parameters, faster training.
    """
...
def train_gru_model(model, X_train, y_train, X_test, y_test,
    ...
    """
    Trains GRU model with early stopping.

    Args:
        model: SOH_GRU instance
...
if __name__ == "__main__":
    print("=" * 60)
    print("BMS Predictive Maintenance - GRU Top30 Pipeline")
    print("=" * 60)
    ...
    # ============================================================
    # STEP 8: GRU Model Training (Baseline)
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 8: GRU Model Training (Baseline)")
    print("=" * 60)
    ...
    # --- Baseline GRU ---
    print("\n  --- Baseline GRU ---")
    print(f"  Architecture: BiGRU, hidden=64, layers=2, dropout=0.2")
    baseline_model = SOH_GRU(n_features, hidden_dim=64, n_layers=2, dropout=0.2)
    t0 = time.time()
    train_losses, test_losses = train_gru_model(
    ...
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X_seq)):
        cv_model = SOH_GRU(n_features, hidden_dim=64, n_layers=2, dropout=0.2)
        train_gru_model(cv_model, X_seq[tr_idx], y[tr_idx], X_seq[te_idx], y[te_idx],
    ...
    # Save baseline model
    torch.save(baseline_model.state_dict(), os.path.join(OUTPUT_DIR, "model_gru_baseline.pt"))
    print(f"\n[MODEL] model_gru_baseline.pt saved")
    ...
    plt.suptitle("STEP 8 - GRU: Train vs Test Metrics (Overfitting Check)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    ax.plot(cycles_test, y_pred_test, "r--s", label="GRU Prediction", linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 8 - GRU SOH Prediction\nMAE={mae_test:.3f}% | RMSE={rmse_test:.3f}% | R2={r2_test:.4f}",
                 fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title(f"STEP 8 - GRU Residuals (max |error|={np.max(np.abs(residuals)):.3f}%)", fontweight="bold")
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title("STEP 8 - GRU Training Loss Curve", fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    for i, (name, val, clr) in enumerate(zip(metric_names, metric_vals, bar_colors_met)):
        ...
        axes[i].grid(True, alpha=0.3, axis="y")
    plt.suptitle("STEP 8 - GRU Test Results", fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    for hd in PARAM_GRID["hidden_dim"]:
        for nl in PARAM_GRID["n_layers"]:
            for lr_val in PARAM_GRID["lr"]:
                model_gs = SOH_GRU(n_features, hidden_dim=hd, n_layers=nl, dropout=0.2)
                train_gru_model(model_gs, X_train, y_train, X_test, y_test,
    ...
    print(f"\n  --- Tuned GRU (hidden={best_params['hidden_dim']}, "
          f"layers={best_params['n_layers']}, lr={best_params['lr']}) ---")
    tuned_model = SOH_GRU(n_features, hidden_dim=best_params["hidden_dim"],
    ...
    cv_maes_tuned = []
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X_seq)):
        cv_model = SOH_GRU(n_features, hidden_dim=best_params["hidden_dim"],
                            n_layers=best_params["n_layers"], dropout=0.2)
        train_gru_model(cv_model, X_seq[tr_idx], y[tr_idx], X_seq[te_idx], y[te_idx],
    ...
    # Save tuned model
    torch.save(tuned_model.state_dict(), os.path.join(OUTPUT_DIR, "model_gru_tuned.pt"))
    print(f"\n[MODEL] model_gru_tuned.pt saved")
    ...
    for i, (name, bl_v, tn_v) in enumerate(zip(metrics_names, baseline_vals, tuned_vals)):
        ...
        for bar, val in zip(bars, [bl_v, tn_v]):
            axes[i].text(bar.get_x() + bar.get_width() / 2, bar.get_height(),
                         f"{val:.4f}", ha="center", va="bottom", fontsize=10)
        axes[i].grid(True, alpha=0.3, axis="y")
    plt.suptitle("STEP 9 - Baseline vs Tuned GRU (Test Metrics)", fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(cycles_test, y_test, "b-o", label="Actual SOH", linewidth=2, markersize=5)
    ax.plot(cycles_test, y_pred_tuned_test, "r--s", label="GRU Tuned Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_tuned_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 9 - Tuned GRU SOH Prediction\n"
    ...
    # --- Result CSV ---
    result_path = os.path.join(OUTPUT_DIR, "Result_GRU_Top30.csv")
    ...
    print(f"[CSV] Result_GRU_Top30.csv saved")

    # --- Final Summary ---
    print("\n" + "=" * 60)
    print("GRU PIPELINE COMPLETED")
    print("=" * 60)
