"""
BMS Predictive Maintenance - CNN Top30 Pipeline
==================================================

OBJECTIVE:
----------
Build an 1D-CNN (1D Convolutional Neural Network) model that predicts SOH from
battery operational data using Top 30 features by SOH correlation.
Unlike classical ML models (RF, XGBoost), 1D-CNN processes sequences
...
8. CNN model training (Baseline + Tuned)
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_CNN_Top30")
...
# CNN sequence length: how many past cycles the model sees for each prediction
SEQ_LEN = 5
...
class SOH_CNN(nn.Module):
    """
    1D Convolutional Neural Network for SOH prediction.

    Architecture:
        Input (seq_len, n_features) -> transpose to (n_features, seq_len)
        -> Conv1d (n_features -> n_filters, kernel=3)
        -> BatchNorm + ReLU
        -> Conv1d (n_filters -> n_filters*2, kernel=3)
        -> BatchNorm + ReLU
        -> AdaptiveAvgPool1d(1)
        -> Dropout
        -> FC head (n_filters*2 -> n_filters -> 1)
    """
    def __init__(self, n_features, seq_len=5, n_filters=64, dropout=0.2):
        super().__init__()
...
if __name__ == "__main__":
    print("=" * 60)
    print("BMS Predictive Maintenance - CNN Top30 Pipeline")
    print("=" * 60)
    ...
    # ============================================================
    # STEP 8: CNN Model Training (Baseline)
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 8: CNN Model Training (Baseline)")
    print("=" * 60)
    ...
    # --- Baseline CNN ---
    print("\n  --- Baseline CNN ---")
    print(f"  Architecture: 1D-CNN, filters=64, dropout=0.2")
    baseline_model = SOH_CNN(n_features, seq_len=SEQ_LEN, n_filters=64, dropout=0.2)
    t0 = time.time()
    train_losses, test_losses = train_cnn_model(
    ...
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X_seq)):
        cv_model = SOH_CNN(n_features, seq_len=SEQ_LEN, n_filters=64, dropout=0.2)
        train_cnn_model(cv_model, X_seq[tr_idx], y[tr_idx], X_seq[te_idx], y[te_idx],
    ...
    # Save baseline model
    torch.save(baseline_model.state_dict(), os.path.join(OUTPUT_DIR, "model_cnn_baseline.pt"))
    print(f"\n[MODEL] model_cnn_baseline.pt saved")
    ...
    plt.suptitle("STEP 8 - CNN: Train vs Test Metrics (Overfitting Check)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    ax.plot(cycles_test, y_pred_test, "r--s", label="CNN Prediction", linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 8 - CNN SOH Prediction\nMAE={mae_test:.3f}% | RMSE={rmse_test:.3f}% | R2={r2_test:.4f}",
                 fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title(f"STEP 8 - CNN Residuals (max |error|={np.max(np.abs(residuals)):.3f}%)", fontweight="bold")
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title("STEP 8 - CNN Training Loss Curve", fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    for i, (name, val, clr) in enumerate(zip(metric_names, metric_vals, bar_colors_met)):
        ...
        axes[i].grid(True, alpha=0.3, axis="y")
    plt.suptitle("STEP 8 - CNN Test Results", fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    print(f"  Grid: {len(PARAM_GRID['n_filters'])} x {len(PARAM_GRID['dropout'])} x {len(PARAM_GRID['lr'])} "
          f"= {len(PARAM_GRID['n_filters']) * len(PARAM_GRID['dropout']) * len(PARAM_GRID['lr'])} combinations")
    ...
    for nf in PARAM_GRID["n_filters"]:
        for dp in PARAM_GRID["dropout"]:
            for lr_val in PARAM_GRID["lr"]:
                model_gs = SOH_CNN(n_features, seq_len=SEQ_LEN, n_filters=nf, dropout=dp)
                train_cnn_model(model_gs, X_train, y_train, X_test, y_test,
                ...
                tuning_results.append({
                    "n_filters": nf, "dropout": dp, "lr": lr_val, "test_mae": gs_mae,
                })
                if gs_mae < best_mae:
                    best_mae = gs_mae
                    best_params = {"n_filters": nf, "dropout": dp, "lr": lr_val}
    ...
    print(f"\n  --- Tuned CNN (filters={best_params['n_filters']}, "
          f"dropout={best_params['dropout']}, lr={best_params['lr']}) ---")
    tuned_model = SOH_CNN(n_features, seq_len=SEQ_LEN, n_filters=best_params["n_filters"],
    ...
    cv_maes_tuned = []
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X_seq)):
        cv_model = SOH_CNN(n_features, seq_len=SEQ_LEN, n_filters=best_params["n_filters"],
                            dropout=best_params["dropout"])
        train_cnn_model(cv_model, X_seq[tr_idx], y[tr_idx], X_seq[te_idx], y[te_idx],
    ...
    # Save tuned model
    torch.save(tuned_model.state_dict(), os.path.join(OUTPUT_DIR, "model_cnn_tuned.pt"))
    print(f"\n[MODEL] model_cnn_tuned.pt saved")
    ...
    for i, (name, bl_v, tn_v) in enumerate(zip(metrics_names, baseline_vals, tuned_vals)):
        ...
        for bar, val in zip(bars, [bl_v, tn_v]):
            axes[i].text(bar.get_x() + bar.get_width() / 2, bar.get_height(),
                         f"{val:.4f}", ha="center", va="bottom", fontsize=10)
        axes[i].grid(True, alpha=0.3, axis="y")
    plt.suptitle("STEP 9 - Baseline vs Tuned CNN (Test Metrics)", fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(cycles_test, y_test, "b-o", label="Actual SOH", linewidth=2, markersize=5)
    ax.plot(cycles_test, y_pred_tuned_test, "r--s", label="CNN Tuned Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_tuned_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 9 - Tuned CNN SOH Prediction\n"
    ...
    # --- Result CSV ---
    result_path = os.path.join(OUTPUT_DIR, "Result_CNN_Top30.csv")
    ...
    print(f"[CSV] Result_CNN_Top30.csv saved")
