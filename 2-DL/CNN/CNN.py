"""
BMS Predictive Maintenance - CNN Pipeline
============================================
...
1D-CNN processes sequences of cycles and can learn temporal patterns
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_CNN")
...
class SOH_CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.conv_block(x).squeeze(-1)  # -> (batch, n_filters*2)
        h = self.dropout(h)
...
if __name__ == "__main__":
    print("=" * 60)
    print("BMS Predictive Maintenance - CNN Pipeline")
    print("=" * 60)
    ...
    # ---- STEP 6: Feature Selection (All Features — CNN selects internally) ----
    print("\n" + "=" * 60)
    print("STEP 6: Feature Selection (All Features)")
    print("=" * 60)
    feature_cols = all_feature_cols  # Use all features — no manual selection
    print(f"  Total features: {len(feature_cols)}")
    print(f"  No manual selection — CNN handles feature importance via learned weights")
    ...
    # --- Result CSV ---
    result_path = os.path.join(OUTPUT_DIR, "Result_CNN.csv")
    ...
    print(f"[CSV] Result_CNN.csv saved")
