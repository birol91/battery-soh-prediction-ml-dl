"""
BMS Predictive Maintenance - MLP Top30 Pipeline
==================================================

OBJECTIVE:
----------
Build an MLP (Multi-Layer Perceptron) model that predicts SOH from
battery operational data using Top 30 features by SOH correlation.
Unlike classical ML models (RF, XGBoost), MLP processes sequences
of cycles and can learn temporal patterns without requiring manually
...
FEATURE SELECTION:
------------------
Top 30 features selected by absolute correlation with SOH label.
This reduces dimensionality while keeping the most informative signals.
...
6. Feature selection (Top 30 by SOH correlation)
7. Feature scaling + sequence preparation
8. MLP model training (Baseline + Tuned)
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_MLP_Top30")
...
# MLP does not use sequences length: how many past cycles the model sees for each prediction
...
# ============================================================
# STEP 7: Feature Scaling (No Sequence — MLP uses flat input)
# ============================================================

def prepare_features_flat(cycle_df: pd.DataFrame, feature_cols: list):
    """
    Prepares data for MLP (flat input, no sequences).
    Unlike LSTM/GRU/CNN, MLP treats each cycle independently.

    Returns:
        X_scaled: np.array of shape (n_samples, n_features) — flat
        y: np.array of shape (n_samples,)
        scaler: fitted StandardScaler
    """
    print("\n" + "=" * 60)
    print("STEP 7: Feature Scaling (Flat Input — No Sequences)")
    print("=" * 60)
    ...
    print(f"  X shape: {X_scaled.shape} (samples, features) — flat, no sequence")
...
class SOH_MLP(nn.Module):
    """
    Multi-Layer Perceptron (fully connected) for SOH prediction.

    Architecture:
        Input (n_features) — flat, no sequence
        -> Linear -> BatchNorm -> ReLU -> Dropout
        -> Linear -> BatchNorm -> ReLU -> Dropout
        -> Linear -> BatchNorm -> ReLU
        -> Linear -> 1

    MLP treats each cycle independently (no temporal context).
    Serves as a DL baseline.
    """
    ...
    def forward(self, x):
        # x shape: (batch, n_features) — flat input
        return self.network(x).squeeze(-1)


def train_mlp_model(model, X_train, y_train, X_test, y_test,
                     epochs=200, batch_size=16, lr=0.001, patience=30):
    """
    Trains MLP model with early stopping.

    Args:
        model: SOH_MLP instance
...
if __name__ == "__main__":
    print("=" * 60)
    print("BMS Predictive Maintenance - MLP Top30 Pipeline")
    print("=" * 60)
    ...
    # ---- STEP 6: Feature Selection (Top 30 by SOH Correlation) ----
    print("\n" + "=" * 60)
    print("STEP 6: Feature Selection (Top 30)")
    print("=" * 60)

    top30_features = list(soh_corr_abs.head(30).index)
    feature_cols = top30_features

    print(f"  Selected top 30 features by absolute SOH correlation:")
    for i, feat in enumerate(feature_cols):
        corr_val = soh_corr[feat]
        print(f"    {i+1:>2}. {feat:<45s} corr={corr_val:+.4f}")

    # Save selected features CSV
    selected_df = cycle_df[["cycle"] + feature_cols + ["soh_label"]].copy()
    selected_df.to_csv(os.path.join(OUTPUT_DIR, "df_step6_selected_features.csv"), index=False)
    print(f"[CSV] df_step6_selected_features.csv saved ({len(feature_cols)} features)")

    # Step 6 visualization: Top 30 correlation bar chart
    top30_signed = soh_corr[feature_cols]
    fig, ax = plt.subplots(figsize=(12, 10))
    colors_bar = ["tab:green" if v > 0 else "tab:red" for v in top30_signed.values]
    ax.barh(range(len(top30_signed)), top30_signed.values, color=colors_bar,
            edgecolor="black", linewidth=0.3)
    ...
    ax.set_yticklabels(top30_signed.index, fontsize=8)
    ax.set_xlabel("Correlation (with SOH)")
    ax.set_title("STEP 6 - Selected Top 30 Features (SOH Correlation)\n(Green=Positive, Red=Negative)",
                 fontsize=13, fontweight="bold")
    ax.axvline(x=0, color="black", linewidth=0.5)
    ax.grid(True, alpha=0.3, axis="x")
    ax.invert_yaxis()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "step6_top30_correlation.png"), dpi=150, bbox_inches="tight")
    plt.close()
    print(f"[PLOT] step6_top30_correlation.png saved")

    # ---- STEP 7: Feature Scaling + Sequence Preparation ----
    X_flat, y, scaler = prepare_features_flat(cycle_df, feature_cols)
    ...
    # ============================================================
    # STEP 8: MLP Model Training (Baseline)
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 8: MLP Model Training (Baseline)")
    print("=" * 60)
    ...
    test_mask = np.zeros(len(X_flat), dtype=bool)
    test_mask[3::4] = True
    ...
    # --- Baseline MLP ---
    print("\n  --- Baseline MLP ---")
    print(f"  Architecture: MLP, hidden=128, layers=3, dropout=0.2")
    baseline_model = SOH_MLP(n_features, hidden_dim=128, dropout=0.2)
    t0 = time.time()
    train_losses, test_losses = train_mlp_model(
    ...
    cv_maes = []
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X_flat)):
        cv_model = SOH_MLP(n_features, hidden_dim=128, dropout=0.2)
        train_mlp_model(cv_model, X_flat[tr_idx], y[tr_idx], X_flat[te_idx], y[te_idx],
                         epochs=150, batch_size=16, lr=0.001, patience=20)
        cv_model.eval()
        with torch.no_grad():
            cv_pred = cv_model(torch.tensor(X_flat[te_idx], dtype=torch.float32).to(DEVICE)).cpu().numpy()
    ...
    # Save baseline model
    torch.save(baseline_model.state_dict(), os.path.join(OUTPUT_DIR, "model_mlp_baseline.pt"))
    print(f"\n[MODEL] model_mlp_baseline.pt saved")
    ...
    plt.suptitle("STEP 8 - MLP: Train vs Test Metrics (Overfitting Check)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    ax.plot(cycles_test, y_pred_test, "r--s", label="MLP Prediction", linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 8 - MLP SOH Prediction\nMAE={mae_test:.3f}% | RMSE={rmse_test:.3f}% | R2={r2_test:.4f}",
                 fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title(f"STEP 8 - MLP Residuals (max |error|={np.max(np.abs(residuals)):.3f}%)", fontweight="bold")
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title("STEP 8 - MLP Training Loss Curve", fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    for i, (name, val, clr) in enumerate(zip(metric_names, metric_vals, bar_colors_met)):
        ...
        axes[i].grid(True, alpha=0.3, axis="y")
    plt.suptitle("STEP 8 - MLP Test Results", fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    print(f"  Grid: {len(PARAM_GRID['hidden_dim'])} x {len(PARAM_GRID['dropout'])} x {len(PARAM_GRID['lr'])} "
          f"= {len(PARAM_GRID['hidden_dim']) * len(PARAM_GRID['dropout']) * len(PARAM_GRID['lr'])} combinations")
    ...
    for hd in PARAM_GRID["hidden_dim"]:
        for dp in PARAM_GRID["dropout"]:
            for lr_val in PARAM_GRID["lr"]:
                model_gs = SOH_MLP(n_features, hidden_dim=hd, dropout=dp)
                train_mlp_model(model_gs, X_train, y_train, X_test, y_test,
                ...
                tuning_results.append({
                    "hidden_dim": hd, "dropout": dp, "lr": lr_val, "test_mae": gs_mae,
                })
                if gs_mae < best_mae:
                    best_mae = gs_mae
                    best_params = {"hidden_dim": hd, "dropout": dp, "lr": lr_val}
    ...
    print(f"\n  --- Tuned MLP (hidden={best_params['hidden_dim']}, "
          f"dropout={best_params['dropout']}, lr={best_params['lr']}) ---")
    tuned_model = SOH_MLP(n_features, hidden_dim=best_params["hidden_dim"],
    ...
    cv_maes_tuned = []
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X_flat)):
        cv_model = SOH_MLP(n_features, hidden_dim=best_params["hidden_dim"],
                            dropout=best_params["dropout"])
        train_mlp_model(cv_model, X_flat[tr_idx], y[tr_idx], X_flat[te_idx], y[te_idx],
    ...
    # Save tuned model
    torch.save(tuned_model.state_dict(), os.path.join(OUTPUT_DIR, "model_mlp_tuned.pt"))
    print(f"\n[MODEL] model_mlp_tuned.pt saved")
    ...
    for i, (name, bl_v, tn_v) in enumerate(zip(metrics_names, baseline_vals, tuned_vals)):
        ...
        for bar, val in zip(bars, [bl_v, tn_v]):
            axes[i].text(bar.get_x() + bar.get_width() / 2, bar.get_height(),
                         f"{val:.4f}", ha="center", va="bottom", fontsize=10)
        axes[i].grid(True, alpha=0.3, axis="y")
    plt.suptitle("STEP 9 - Baseline vs Tuned MLP (Test Metrics)", fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(cycles_test, y_test, "b-o", label="Actual SOH", linewidth=2, markersize=5)
    ax.plot(cycles_test, y_pred_tuned_test, "r--s", label="MLP Tuned Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_tuned_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 9 - Tuned MLP SOH Prediction\n"
    ...
    # --- Result CSV ---
    result_path = os.path.join(OUTPUT_DIR, "Result_MLP_Top30.csv")
    ...
    print(f"[CSV] Result_MLP_Top30.csv saved")
