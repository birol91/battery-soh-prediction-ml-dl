"""
BMS Predictive Maintenance - MLP Pipeline
============================================
...
MLP processes sequences of cycles and can learn temporal patterns
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_MLP")
...
def prepare_features_flat(cycle_df: pd.DataFrame, feature_cols: list):
    ...
    No sliding window or sequence creation needed.
    ...
    print(f"  Note: MLP treats each cycle independently (no temporal context)")
...
class SOH_MLP(nn.Module):
    ...
    This serves as a DL baseline — if sequence models (LSTM, GRU)
    don't beat MLP, the temporal information isn't helping.
    """
    ...
    def forward(self, x):
        # x shape: (batch, n_features) — flat input, no sequence
...
if __name__ == "__main__":
    print("=" * 60)
    print("BMS Predictive Maintenance - MLP Pipeline")
    print("=" * 60)
    ...
    # ---- STEP 6: Feature Selection (All Features — MLP selects internally) ----
    print("\n" + "=" * 60)
    print("STEP 6: Feature Selection (All Features)")
    print("=" * 60)
    feature_cols = all_feature_cols  # Use all features — no manual selection
    print(f"  Total features: {len(feature_cols)}")
    print(f"  No manual selection — MLP handles feature importance via learned weights")
    ...
    # --- Result CSV ---
    result_path = os.path.join(OUTPUT_DIR, "Result_MLP.csv")
    ...
    print(f"[CSV] Result_MLP.csv saved")
