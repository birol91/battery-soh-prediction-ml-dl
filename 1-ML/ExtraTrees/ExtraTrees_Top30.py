...
6. Train ML models (XGBoost, Extra Trees, etc.)
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_ExtraTrees_Top30")
    ...
    # ============================================================
    # STEP 8: Model Training (Extra Trees)
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 8: Model Training (Extra Trees)")
    print("=" * 60)
    ...
    cv_scores_rf = cross_val_score(
        ExtraTreesRegressor(n_estimators=300, max_depth=None, min_samples_split=2,
    ...
    plt.suptitle("STEP 8 - Extra Trees: Train vs Test Metrics (Overfitting Check)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    ax.plot(cycles_test, y_pred_rf_test, "r--s", label="ET Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_rf_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 8 - Extra Trees SOH Prediction\n"
    ...
    ax.set_title(f"STEP 8 - Extra Trees Residuals (max |error|={np.max(np.abs(residuals)):.3f}%)",
                 fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title("STEP 8 - Extra Trees Feature Importance (Top 30)",
                 fontweight="bold")
    ax.grid(True, alpha=0.3, axis="x")
    plt.tight_layout()
    ...
    plt.suptitle("STEP 8 - Extra Trees Test Results",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    ...
    # Save model weights
    joblib.dump(rf_model, os.path.join(OUTPUT_DIR, "model_et_baseline.joblib"))
    print(f"[MODEL] model_et_baseline.joblib saved")
    ...
    for fold_i, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        rf_cv = ExtraTreesRegressor(**gs.best_params_, random_state=42, n_jobs=-1)
    ...
    plt.suptitle("STEP 9 - Baseline vs Tuned Extra Trees (Test Metrics)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(14, 6))
    ax.plot(cycles_test, y_test, "b-o", label="Actual SOH", linewidth=2, markersize=5)
    ax.plot(cycles_test, y_pred_tuned_test, "r--s", label="ET Tuned Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_tuned_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 9 - Tuned Extra Trees SOH Prediction\n"
    ...
    # Save tuned model
    joblib.dump(rf_tuned, os.path.join(OUTPUT_DIR, "model_et_tuned.joblib"))
    print(f"[MODEL] model_et_tuned.joblib saved")
    ...
    result_path = os.path.join(OUTPUT_DIR, "Result_ExtraTrees_Top30.csv")
    ...
    print(f"[CSV] Result_ExtraTrees_Top30.csv saved")
