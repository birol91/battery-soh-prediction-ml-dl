...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_XGBoost_Top30")
    ...
    ax.set_title("STEP 8 - XGBoost Feature Importance (Top 30)",
                 fontweight="bold")
    ax.grid(True, alpha=0.3, axis="x")
    plt.tight_layout()
    ...
    # Save model weights
    joblib.dump(rf_model, os.path.join(OUTPUT_DIR, "model_xgb_baseline.joblib"))
    ...
    for fold_i, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        rf_cv = XGBRegressor(**gs.best_params_, colsample_bytree=0.8, reg_alpha=0.1,
                            reg_lambda=1.0, random_state=42, verbosity=0)
    ...
    # Save tuned model
    joblib.dump(rf_tuned, os.path.join(OUTPUT_DIR, "model_xgb_tuned.joblib"))
    ...
    result_path = os.path.join(OUTPUT_DIR, "Result_XGBoost_Top30.csv")
    ...
    print(f"[CSV] Result_XGBoost_Top30.csv saved")
