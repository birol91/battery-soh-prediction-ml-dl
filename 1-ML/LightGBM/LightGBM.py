...
Step 1-5: Data loading, feature extraction, trend.
Step 6: Correlation analysis (informational only, no elimination).
Step 7: All features are fed to the model (feature selection is left to the model).
Step 8-9: Training with LightGBM model and hyperparameter tuning.
"""
...
# Voltage signals arrive in mV, will remain in mV
...
def load_mat_to_dataframe(mat_path: str) -> pd.DataFrame:
    """
    Reads the Simulink Recording .mat file and converts it to a pandas DataFrame.
...
# ============================================================
# STEP 1 (cont.): Visualize Raw Signals
# ============================================================
...
def detect_cycle_boundaries(df: pd.DataFrame,
    ...
    From one peak to the next peak = 1 full cycle.
    ...
    enter_high = np.where(transitions == 1)[0]   # going above threshold (charge complete)
    exit_high = np.where(transitions == -1)[0]    # going below threshold (discharge starting)
    ...
    # Find the SOC peak point in each high region
...
# Raw signal columns for intra-cycle statistics
RAW_SIGNALS = [
...
def compute_cycle_summary(df: pd.DataFrame, start: int, end: int, cycle_num: int) -> dict:
    """
    Summarizes a single cycle's 1 Hz data into one row.
    ...
    for sig in RAW_SIGNALS:
        ...
        # Discharge end value (at the moment SOC is minimum)
        row[f"{sig}_discharge_end"] = vals[soc_min_idx]
        # Charge start value (first value from SOC minimum onwards)
    ...
    #    As cells age, they become more imbalanced -> spread increases
    ...
    if np.any(meaningful_mask):
        ...
        row["voltage_asymmetry_mean"] = 1.0  # cells equal -> symmetric

    # 3. Voltage spread under load (active moments: |Current| > 0.5A)
    #    Excludes rest moments and calculates spread only under load
    ...
    #    As cells age, internal resistance increases -> heat increases -> spread increases
    ...
    #     Excludes rest moments and calculates thermal spread only under load
    if np.any(active_mask):
    ...
    # 7. Charge time: Durations in the charge phase (seconds)
    #    As cells age, charge time increases
    ...
    if len(t_spread) > W and len(abs_current) > W:
        ...
        # Moments that are not NaN and have meaningful current change
...
# Core features for trend calculation
TREND_FEATURES = [
...
def add_trend_features(cycle_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Linear slope of proxy_IR_mean over the last 10 cycles (internal resistance increase rate)
    ...
    # Cycle number is unknown in real-world scenarios, feeding it to the model would be data leakage.

    n_after = len(df.columns)
    print(f"\n  {n_before} features -> {n_after} features (+{n_after - n_before} trend features)")
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_LightGBM")
    ...
    # ---- Domain-Specific Feature Charts ----
    ...
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 7: All Features Will Be Used (LightGBM selects its own)")
    print("=" * 60)
    print(f"\n  Total feature count: {len(feature_cols)}")
    print(f"  No feature selection performed — LightGBM makes its own selection via feature_importance")
    ...
    # Separate Features (X) and Target (y) — all features are used
    ...
    # --- Sample Weight: more weight to lower SOH region ---
    ...
    ax.set_title(f"STEP 8 - LightGBM Feature Importance (Top {top_n} / {len(feature_cols)})",
                 fontweight="bold")
    ...
    if mae_tuned_test < mae_test:
        ...
        print(f"  [WARNING] Grid Search did not provide improvement. Baseline model should be used.")

    # --- Result CSV: Collect all results in a single file ---
    result_path = os.path.join(OUTPUT_DIR, "Result_LightGBM.csv")
    ...
    print(f"[CSV] Result_LightGBM.csv saved")
