"""
BMS Predictive Maintenance - LightGBM Pipeline (Birol)
======================================================

Step 1-7: Same as LightGBM pipeline (data loading, feature extraction,
           trend, correlation, feature selection).
Step 8-9: Training and hyperparameter tuning with LightGBM model.
"""
...
def load_mat_to_dataframe(mat_path: str) -> pd.DataFrame:
    """
    Reads Simulink Recording .mat file and converts to pandas DataFrame.
...
def detect_cycle_boundaries(df: pd.DataFrame,
    ...
    # Find entry/exit points of regions where SOC rises above threshold
    above = soc >= soc_high_thresh
    transitions = np.diff(above.astype(int))
    enter_high = np.where(transitions == 1)[0]   # rises above threshold (charge completed)
    exit_high = np.where(transitions == -1)[0]    # drops below threshold (discharge starts)
    ...
    # But a more robust approach: find the start of each high region

    # Find SOC peak point in each high region
    # High region = consecutive regions where SOC >= soc_high_thresh
    # Peak point = end point of high region (just before discharge starts)
    raw_peaks = []

    # If data starts with high, the first exit_high is a peak
    if soc[0] >= soc_high_thresh and len(exit_high) > 0:
        raw_peaks.append(exit_high[0])

    # Subsequent peaks: exit_high after enter_high
    ...
    # Each cycle: starts from SOC ~100% -> drops to 10% -> rises back to 100%
    boundaries = []
...
# ============================================================
# STEP 4: Cycle-Based Summarization (Feature Extraction)
# ============================================================

# Raw signal columns for within-cycle statistics
RAW_SIGNALS = [
...
def compute_cycle_summary(df: pd.DataFrame, start: int, end: int, cycle_num: int) -> dict:
    ...
    for sig in RAW_SIGNALS:
        ...
        # End-of-discharge value (at SOC minimum)
        row[f"{sig}_discharge_end"] = vals[soc_min_idx]
        # Charge start value (first value after SOC minimum)
    ...
    # 1. Voltage spread (inter-cell imbalance)
    #    As cells age, imbalance increases -> spread grows
    ...
    #    1.0 = symmetric, >1 = one cell above, <1 = one cell below
    ...
    #    Removes rest moments and calculates spread only under load
    ...
    #     Asymmetry only during moments under load with meaningful spread
    ...
    #    As cells age, internal resistance increases -> heat increases -> spread grows
    ...
    #     Removes rest moments and calculates thermal spread only under load
    if np.any(active_mask):
    ...
    #    As cells age, internal resistance increases -> this value rises
    di = np.diff(current)
    dv = np.diff(v_avg)
    ...
    #    As cells age, losses increase -> efficiency decreases
    ...
    if np.any(soc_increasing):
        ...
        if avg_current_when_soc_up < 0:
            # Reversed convention: negative = charge, positive = discharge
    ...
    #    As cells age, charge duration increases
...
def summarize_cycles(df: pd.DataFrame, boundaries: list) -> pd.DataFrame:
    ...
    """
    print("\n" + "=" * 60)
    print("STEP 4: Cycle-Based Summarization (Feature Extraction)")
    print("=" * 60)
...
# Base features for trend calculation
TREND_FEATURES = [
...
def add_trend_features(cycle_df: pd.DataFrame) -> pd.DataFrame:
    ...
    - Rolling mean and std (window: 3, 5, 10)
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_LightGBM_Top30")
    ...
    # Cycle vs SOH and Cycle vs Duration charts
    ...
    # ---- STEP 4: Cycle-based summarization ----
    cycle_df = summarize_cycles(df, boundaries)

    # STEP 4 CSV: Cycle-based features
    ...
    # ---- Domain-Specific Feature Grafikleri ----
    ...
    # 6. Proxy IR Trend Slope (son 10 cycle lineer egim)
    ...
    # 7. Energy Efficiency - Delta (cycle-to-cycle degisim)
    ...
    # 9. Thermal Spread - Rolling Std (kararlilik)
    ...
    # Chart: Top 30 Feature SOH Correlation (horizontal bar chart)
    ...
    # ============================================================
    # STEP 8: Model Training (LightGBM)
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 8: Model Training (LightGBM)")
    print("=" * 60)
    ...
    # --- Train / Test Split: Interleaved (every 4th cycle is test) ---
    ...
    # --- Sample Weight: higher weight for low SOH region ---
    ...
    plt.suptitle("STEP 8 - LightGBM: Train vs Test Metrics (Overfitting Check)",
    ...
    ax.set_title(f"STEP 8 - LightGBM Residuals (max |error|={np.max(np.abs(residuals)):.3f}%)",
                 fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title("STEP 8 - LightGBM Feature Importance (Top 30)",
                 fontweight="bold")
    ax.grid(True, alpha=0.3, axis="x")
    plt.tight_layout()
    ...
    plt.suptitle("STEP 8 - LightGBM Test Results",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    ...
    # Save model weights
    joblib.dump(lgbm_model, os.path.join(OUTPUT_DIR, "model_lgbm_baseline.joblib"))
    print(f"[MODEL] model_lgbm_baseline.joblib saved")
    ...
    gs = GridSearchCV(
        LGBMRegressor(colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0,
    ...
    y_pred_tuned_test = lgbm_tuned.predict(X_test)
    y_pred_tuned_train = lgbm_tuned.predict(X_train)
    ...
    for fold_i, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        lgbm_cv = LGBMRegressor(**gs.best_params_, colsample_bytree=0.8,
                               reg_alpha=0.1, reg_lambda=1.0,
                               random_state=42, verbose=-1)
        lgbm_cv.fit(X_cv_train, y_cv_train)

        y_cv_pred_test = lgbm_cv.predict(X_cv_test)
    ...
    # --- Chart 1: Baseline vs Tuned Bar Chart ---
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    ...
    plt.suptitle("STEP 9 - Baseline vs Tuned LightGBM (Test Metrics)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    # --- Chart 2: Tuned SOH Prediction vs Actual ---
    fig, ax = plt.subplots(figsize=(14, 6))
    ax.plot(cycles_test, y_test, "b-o", label="Actual SOH", linewidth=2, markersize=5)
    ax.plot(cycles_test, y_pred_tuned_test, "r--s", label="LGBM Tuned Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_tuned_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 9 - Tuned LightGBM SOH Prediction\n"
    ...
    # Save tuned model
    joblib.dump(lgbm_tuned, os.path.join(OUTPUT_DIR, "model_lgbm_tuned.joblib"))
    print(f"[MODEL] model_lgbm_tuned.joblib saved")
    ...
    result_path = os.path.join(OUTPUT_DIR, "Result_LightGBM_Top30.csv")
    ...
    print(f"[CSV] Result_LightGBM_Top30.csv saved")
