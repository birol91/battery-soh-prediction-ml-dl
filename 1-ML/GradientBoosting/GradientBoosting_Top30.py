...
PROBLEM DEFINITION:
-------------------
We have a Li-ion battery pack (LG 18650HG2 NMC cells) modeled in
Simulink with a 5s1p (5 series cells, 1 parallel) configuration.

The battery is repeatedly charged/discharged with a UDDS (Urban
Dynamometer Driving Schedule) drive cycle. With each cycle, the battery
ages further (capacity decreases, internal resistance increases).
...
SOH label is computed from the Simscape Fade formula (ground truth).
In the real world, this formula is used instead of RPT (Reference Performance Test).
...
6. Train ML models (XGBoost, Gradient Boosting, etc.)
...
# Voltage signals come in mV, will remain as mV
...
def load_mat_to_dataframe(mat_path: str) -> pd.DataFrame:
    ...
    The .mat file is in HDF5 format. Inside, each signal is stored
    as a separate stream under #sigstream#.
...
def detect_cycle_boundaries(df: pd.DataFrame,
    ...
    From one peak point to the next peak point = 1 complete cycle.

    Args:
        df: Raw data DataFrame (must have SOC column)
        soc_high_thresh: Threshold for SOC to be considered "fully charged" (%)
    ...
    # Find entry/exit points of regions where SOC goes above threshold
    above = soc >= soc_high_thresh
    transitions = np.diff(above.astype(int))
    enter_high = np.where(transitions == 1)[0]   # goes above threshold (charge completed)
    exit_high = np.where(transitions == -1)[0]    # goes below threshold (discharge starting)

    # If data starts at SOC=100%, the first "high" region starts without a transition
    # In that case we can use the first exit_high point
    # But the more robust way: find the start of each high region
    ...
    # Filter out peak points that are too close together
    filtered_peaks = []
    for pk in raw_peaks:
    ...
    # Last segment: from last peak to end of data (may be an incomplete cycle)
...
# Raw signal columns for which in-cycle statistics will be extracted
RAW_SIGNALS = [
...
def compute_cycle_summary(df: pd.DataFrame, start: int, end: int, cycle_num: int) -> dict:
    ...
    for sig in RAW_SIGNALS:
        ...
        # Discharge end value (at SOC minimum point)
        row[f"{sig}_discharge_end"] = vals[soc_min_idx]
        # Charge start value (first value from SOC minimum onward)
    ...
    #    As battery ages, cells become imbalanced -> spread increases
    ...
    #    Excludes rest moments, computes spread only under load
    ...
    #     Excludes rest moments, computes thermal spread only under load
    if np.any(active_mask):
    ...
    #    Estimates internal resistance from voltage response during large current changes
    ...
    if np.sum(large_di_mask) > 5:
        ...
        proxy_ir_values = proxy_ir_values[proxy_ir_values < 1.0]  # filter out values above 1 Ohm
    ...
    # Current convention check: verify current direction when SOC is increasing
    dsoc = np.diff(soc)
    ...
    # 7. Charge time: Durations during charge phase (seconds)
    ...
    #     Computed over 60-second window averages
    #     Thermal effect of current changes -> increases with aging
...
def add_trend_features(cycle_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Cycle number is unknown in the real world, giving it to the model would be data leakage.
...
if __name__ == "__main__":
    print("=" * 60)
    print("BMS Predictive Maintenance - Phase 1 Pipeline")
    print("=" * 60)

    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    OUTPUT_DIR = os.path.join(SCRIPT_DIR, "Output_GradientBoosting_Top30")
    ...
    # STEP 2 CSV: Voltage-converted data
    ...
    # STEP 2 Visual: Voltage signals after V conversion
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    time_hours = df["timestamp"].values / 3600.0
    v_signals = ["Cell_V_Max", "Cell_V_Min", "Cell_V_Avg"]
    colors_v = ["tab:red", "tab:cyan", "tab:blue"]
    for ax, sig, color in zip(axes, v_signals, colors_v):
    ...
    # STEP 6 Visual: SOH correlation distribution of all features
    fig, ax = plt.subplots(figsize=(10, 6))
    corr_values = soh_corr_abs.values
    ax.hist(corr_values, bins=20, color="#6BAED6", edgecolor="black", linewidth=0.5)
    ax.axvline(x=0.8, color="red", linestyle="--", linewidth=1.5, label="High correlation threshold (0.8)")
    ax.set_xlabel("|Correlation| (with SOH)")
    ax.set_ylabel("Feature Count")
    ax.set_title(f"STEP 6 - Feature SOH Correlation Distribution ({len(corr_values)} features)",
                 fontsize=13, fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "step6_correlation_distribution.png"), dpi=150, bbox_inches="tight")
    plt.close()

    # ============================================================
    # STEP 7: Feature Selection - Top 30
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 7: Feature Selection - Top 30")
    print("=" * 60)

    top30_features = soh_corr_abs.head(30).index.tolist()
    top30_corr = soh_corr[top30_features]

    print(f"\n  Top 30 features with highest SOH correlation:")
    for i, feat in enumerate(top30_features):
        direction = "+" if soh_corr[feat] > 0 else "-"
        print(f"  {i+1:>3}. {feat:<50s} {direction}{soh_corr_abs[feat]:.4f}")

    # Save CSV
    selected_cols = ["cycle"] + top30_features + ["soh_label"]
    selected_df = cycle_df[selected_cols].copy()
    selected_df.to_csv(os.path.join(OUTPUT_DIR, "df_step7_selected_features.csv"), index=False)
    print(f"\n[CSV] df_step7_selected_features.csv saved "
          f"({selected_df.shape[0]} cycles x {selected_df.shape[1]} columns)")

    # Visual: Top 30 Feature SOH Correlation (horizontal bar chart)
    fig, ax = plt.subplots(figsize=(12, 10))
    colors = ["tab:green" if v > 0 else "tab:red" for v in top30_corr.values]
    y_pos = range(len(top30_corr))
    ax.barh(y_pos, top30_corr.values, color=colors, edgecolor="black", linewidth=0.3)
    ax.set_yticks(y_pos)
    ax.set_yticklabels(top30_corr.index, fontsize=8)
    ax.set_xlabel("Correlation (with SOH)")
    ax.set_title("STEP 7 - Top 30 Feature SOH Correlation\n(Green=Positive, Red=Negative)",
                 fontweight="bold")
    ax.axvline(x=0, color="black", linewidth=0.5)
    ax.grid(True, alpha=0.3, axis="x")
    ax.invert_yaxis()
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "step7_top30_correlation.png"), dpi=150, bbox_inches="tight")
    plt.close()
    print(f"[PLOT] step7_top30_correlation.png")




    # ============================================================
    # STEP 8: Model Training (Gradient Boosting)
    # ============================================================
    print("\n" + "=" * 60)
    print("STEP 8: Model Training (Gradient Boosting)")
    print("=" * 60)

    # Separate Feature (X) and Target (y)
    X = selected_df[top30_features].values
    y = selected_df["soh_label"].values
    cycles_arr = selected_df["cycle"].values
    ...
    plt.suptitle("STEP 8 - Gradient Boosting: Train vs Test Metrics (Overfitting Check)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    ax.plot(cycles_test, y_pred_rf_test, "r--s", label="GB Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_rf_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 8 - Gradient Boosting SOH Prediction\n"
    ...
    ax.set_title(f"STEP 8 - Gradient Boosting Residuals (max |error|={np.max(np.abs(residuals)):.3f}%)",
                 fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    feat_imp = sorted(zip(top30_features, importances), key=lambda x: x[1], reverse=True)
    ...
    ax.set_title("STEP 8 - Gradient Boosting Feature Importance (Top 30)",
                 fontweight="bold")
    ax.grid(True, alpha=0.3, axis="x")
    plt.tight_layout()
    ...
    plt.suptitle("STEP 8 - Gradient Boosting Test Results",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    ...
    # Save model weights
    joblib.dump(rf_model, os.path.join(OUTPUT_DIR, "model_gb_baseline.joblib"))
    print(f"[MODEL] model_gb_baseline.joblib saved")
    ...
    y_pred_tuned_test = rf_tuned.predict(X_test)
    y_pred_tuned_train = rf_tuned.predict(X_train)
    ...
    print(f"\n  --- 5-Fold CV Detailed Analysis (Tuned Model) ---")
    ...
    for fold_i, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        rf_cv = GradientBoostingRegressor(**gs.best_params_, random_state=42)
    ...
    plt.suptitle("STEP 9 - Baseline vs Tuned Gradient Boosting (Test Metrics)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(14, 6))
    ax.plot(cycles_test, y_test, "b-o", label="Actual SOH", linewidth=2, markersize=5)
    ax.plot(cycles_test, y_pred_tuned_test, "r--s", label="GB Tuned Prediction",
            linewidth=1.5, markersize=5, alpha=0.8)
    ax.fill_between(cycles_test, y_test, y_pred_tuned_test, alpha=0.15, color="red")
    ax.set_xlabel("Cycle")
    ax.set_ylabel("SOH (%)")
    ax.set_title(f"STEP 9 - Tuned Gradient Boosting SOH Prediction\n"
    ...
    # Save tuned model
    joblib.dump(rf_tuned, os.path.join(OUTPUT_DIR, "model_gb_tuned.joblib"))
    print(f"[MODEL] model_gb_tuned.joblib saved")
    ...
    if mae_tuned_test < mae_test:
        ...
        print(f"  [WARNING] Grid Search did not improve performance. Baseline model should be used.")

    # --- Result CSV: Collect all results in one file ---
    result_path = os.path.join(OUTPUT_DIR, "Result_GradientBoosting_Top30.csv")
    ...
    print(f"[CSV] Result_GradientBoosting_Top30.csv saved")
